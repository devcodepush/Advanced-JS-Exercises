...
/* MAP
1. Write a function called capitalize() that takes a string and uses .map to return the same string in all caps. 
...
/* 2. Now write a new function called swapCase() that takes a string of words and uses .map and function capitalize(string) to return a string where every other word is in all caps. 
 */
...
/* 3. Write a function shiftLetters() that takes a string and uses .map to return an encoded string with each letter shifted down the alphabet by one. Hint: Use Look up the JS functions String.fromCharCode() and String.CharCodeAt() and see if you can use Ascii code to acomplish this. 
 */
...
/* REDUCE
4. Write a function counterLetters() that takes a string and returns an object representing the character count for each letter. Use .reduce to build this object. 
...
/* 5. Write a function isPresent() that takes a string and a target, and returns true or false if the target is present in the string. Use .reduce to acomplish this. 
